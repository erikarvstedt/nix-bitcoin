...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      "d '${cfg.dataDir}' 0700 ${config.users.users.lightning-charge.name} ${config.users.users.lightning-charge.group} - -"
    ];
    ...
    systemd.services.lightning-charge = {
      ...
      preStart = ''
        ...
        if [[ -e ${config.services.clightning.dataDir}/lightning-charge.db ]]; then
          ...
          chown ${config.users.users.lightning-charge.name}:${config.users.users.lightning-charge.group} ${cfg.dataDir}/lightning-charge.db
    ...
    nix-bitcoin.secrets.lightning-charge-env.user = "${config.users.users.lightning-charge.name}";
  };
}
