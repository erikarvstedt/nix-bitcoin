  ...
  config = mkIf cfg.enable {
	...
	map $cookie_lang $lang {
      ...
      virtualHosts."mempool" = {
        ...
        extraConfig = ''
          ...
          location /api/v1/ws {
                  proxy_pass http://${cfg.address}:${toString cfg.backendPort}/;
                  proxy_http_version 1.1;
                  proxy_set_header Upgrade $http_upgrade;
                  proxy_set_header Connection "Upgrade";
          }
          location /api/v1 {
                  proxy_pass http://${cfg.address}:${toString cfg.backendPort}/api/v1;
          }
          location /api/ {
                  proxy_pass http://${cfg.address}:${toString cfg.backendPort}/api/v1/;
          }
    ...
    nix-bitcoin.generateSecretsCmds.mempool = ''
      ...
      if [[ mempool-db-password -nt mempool-db-initialScript ]]; then
         echo "grant all on mempool.* to '${cfg.user}'@'localhost' identified by '$(cat mempool-db-password)'" > mempool-db-initialScript
      fi
    '';
  };
}
