...
let
  ...
  bin = pkgs.writeScriptBin "lndconnect-onion" ''
  ...
  options.services.lnd.restOnionService = {
    enable = mkOption {
      ...
      description = ''
        ...
        Add a `lndconnect-onion` binary (https://github.com/LN-Zap/lndconnect) to the system environment.
        This binary generates QRCodes or URIs for connecting applications to lnd via the REST onion service.
      '';
    };
    package = mkOption {
      type = types.package;
