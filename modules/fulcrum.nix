  ...
  options.services.fulcrum = {
    enable = mkOption {
      ...
      description = ''
        ...
        This module disables peering (a fulcrum-native feature for maintaining
        a distributed list of electrum servers), but you can manually enable
        it via option `extraConfig`.
      '';
    };
    address = mkOption {
      type = types.str;
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      { assertion =
        ...
        message = ''
          Fulcrum and Electrs can't both bind to TCP RPC port 50001.
          Change `services.electrs.port` or `services.fulcrum.port`
          to a port other than 50001.
        '';
      }
    ];
